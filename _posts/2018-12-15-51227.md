...
<pre><code>select users.name,users.student_id,users.type,access_log.time as time,access_log.state from users join access_log on users.idm=access_log.idm where time = (select MAX(time) from access_log where users.idm=access_log.idm);
</code></pre>
