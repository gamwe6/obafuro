...
<pre><code>pyenv install 3.6.4
</code></pre>

<p>とすると、下記のようなエラーになります。</p>

<pre><code>BUILD FAILED (OS X 10.13.2 using python-build 20160602)
...
<pre><code>CFLAGS="-I$(xcrun --show-sdk-path)/usr/include" pyenv install 3.6.4
</code></pre>

<p>これでもエラーになります。</p>

<pre><code>CFLAGS="-I$(brew --prefix openssl)/include" LDFLAGS="-L$(brew --prefix openssl)/lib" pyenv install -v 3.6.4
</code></pre>
