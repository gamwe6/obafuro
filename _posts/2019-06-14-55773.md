...
<pre><code>   UPDATE `table名` SET `column名` = replace(`column名`, 'post_date', 'create_date');
</code></pre>
...
<pre><code>alter table hoge change column post_date create_date date;
