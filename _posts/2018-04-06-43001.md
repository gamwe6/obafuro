...
<pre><code>/usr/bin/ld: /usr/local/lib/libcrypto.a(cryptlib.o): relocat R_X86_64_32     against `.rodata.str1.1' can not be used when making a shared     object; recompile with -fPIC
...
<pre><code>ptions used to compile and link:
  with wallet   = yes
  with gui / qt = no
  with zmq      = no
  with test     = yes
