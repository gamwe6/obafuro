...
<pre><code>https://repository.sonatype.org/service/local/artifact/maven/redirect?r=central-proxy&amp;g=org.elasticsearch&amp;a=elasticsearch&amp;v=2.2.1&amp;e=jar
</code></pre>

<p>この url に対して、curl を実行したところ、次のテキストファイルがダウンロードされました。<br>
(実行コマンド: <code>curl -o elasticsearch.jar "$URL"</code>)</p>

<pre><code>If you are not automatically redirected use this url: https://repository.sonatype.org/service/local/repositories/central-proxy/content/org/elasticsearch/elasticsearch/2.2.1/elasticsearch-2.2.1.jar
</code></pre>
