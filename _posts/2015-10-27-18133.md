...
<pre><code>angular.module("myApp").controller "shopEdit", ($scope, Shop, Shelf, Item) -&gt;
...
<pre><code>angular.module('myApp').factory 'Shop', ($resource) -&gt;
  $resource('/api/shops/:id', { id: @id }, { update: { method: 'PUT' }})
</code></pre>

<p>《service》item.coffee</p>

<pre><code>angular.module('myApp').factory 'Item', ($resource) -&gt;
...
<pre><code>$resource('/api/shops/:shop_id/shelfs/:shop_id/items/:id',
  { shop_id: '13495' },
  { update: { method: 'PUT' }})
</code></pre>
