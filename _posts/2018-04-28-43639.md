...
<pre><code>[debug] application - Directory for migration files found. db/migration/default
...
<pre><code> case class CustomerJson(
  id: Option[String],
  name: String,
  sexType: Int,
  zipCode1: String,
...
<pre><code>val point = SQLSyntax.createUnsafely(s"PointFromText(POINT($lat $long))")
</code></pre>
