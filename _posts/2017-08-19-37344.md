...
<pre><code>body_measurement_1  = body_measurements.where(measuring_date_on: "#{@student.birthday &gt;&gt; 0}".."#{@student.birthday &gt;&gt; 1}")&amp;.last
...
<pre><code>BodyMeasurement Load (1.1ms)  SELECT  `body_measurements`.* FROM `body_measurements` WHERE `body_measurements`.`student_id` = 5 AND (`body_measurements`.`measuring_date_on` BETWEEN '2012-12-19' AND '2013-01-19') ORDER BY `body_measurements`.`id` DESC LIMIT 1
