...
<pre><code>//@Pattern(regexp="(\\d{4}/\\d{2}/\\d{2})", message="{actor.validation.birthday}")
...
<pre><code>@GetMapping("/actor/create")
...
<pre><code>&lt;form role="form" action="/actor/save" th:action="@{/actor/save}" th:object="${actorForm}"
          method="post"&gt;
