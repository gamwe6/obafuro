...
<pre><code>[vagrant@localhost ~]$ ssh -T git@gitlab.com
...
<pre><code>[vagrant@localhost ~]$ ssh-keygen -C "ログイン時のアドレス@fuga.com" -t rsa
</code></pre>
...
<pre><code>[vagrant@localhost .ssh]$ ssh -T git@gitlab.com
Permission denied (publickey).
</code></pre>

<p>上記で作成した公開鍵、秘密鍵は以前同じ名前で作成した鍵が存在したので</p>

<pre><code>/home/vagrant/.ssh/id_rsa already exists.
Overwrite (y/n)? y
</code></pre>
