...
<pre><code>public enum ErrorType
{
...
<pre><code>※GetCode(Enum)はEnum型からコードを返すメソッドとする。
...
<pre><code>public class ErrorConst
{
...
<pre><code>if(CustomException.ErrorCode == ErrorConst.Error1Code )
{
    //エラーコードに沿ったエラーメッセージを表示する。
}
else
