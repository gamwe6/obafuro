...
<pre><code>Creating .build/goshim.tar.bz2
...
<pre><code>GOSHIM_DEPS = $(shell ./scripts/goListFiles.sh $(PKGNAME)/core/chaincode/shim)
...
<pre><code>@./scripts/goListFiles.sh $(PKGNAME)/core/chaincode/shim | sed s#$(GOPATH)/src/##g | xargs tar -jhc -C $(GOPATH)/src &gt; $@
</code></pre>
